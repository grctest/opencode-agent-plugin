...
- **Salience: four surfaces carry the mandatory patch.** The paper makes it structurally impossible to omit (App. A.4 requires `state_patch` inside every response's JSON block), so our tool-channel split must recover that weight in prompt form. Production measurement before this change was **1 applied patch across 9 primary turns**, with the two strongest positions in the prompt both silent on it:
  ...
  3. **Self-interest hook** in the guidance bullet: a turn that reasons well but patches nothing "has wasted the work". This is literally true here, because bounded Σ with no transcript replay means unpatched reasoning is genuinely gone — the model is told a fact, not a preference.
  4. **Pass exemption in WHEN TO PASS**: passing is the one turn that does not require the call, which preserves `exempt_pass` semantics and avoids burning a call when the agent has nothing new.
  The executor's patch-retry is the backstop, deliberately **not** the primary mechanism: a strong instruction that only fires after failure inverts the emphasis and teaches the model to ignore the first, softer signal. Measure the effect with the per-turn outcome enum (§12) — `never_attempted` should fall sharply.
